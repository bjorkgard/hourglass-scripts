...
def gruppera(poster: Iterable[Dict[str, str]]) -> List[Tuple[str, List[Dict[str, str]]]]:
    ...
    resultat = []
    for overseer in sorted(grupper, key=svensk_sortnyckel):
        medlemmar = sorted(
            grupper[overseer],
            key=lambda r: (
...
def skapa_pdf(csv_sokvag: Path, output: Path, designval: str, kt: Dict[str, str]):
...
def main():
    ...
    try:
        skapa_pdf(csv_sokvag, output, designval, kt)
